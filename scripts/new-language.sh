...
# Creates: rule file, cursor rule, 3 agent files, config.json entries.
...
# Add agent group
UPDATED=$(jq --arg group "$DISPLAY_NAME" --arg lang "$LANG" \
  '.agents += [{"group": $group, "roles": [{"name": ($lang + "-coder")}, {"name": ($lang + "-reviewer")}, {"name": ($lang + "-tester")}]}]' \
  "$CONFIG_FILE")
echo "$UPDATED" > "$CONFIG_FILE"

# Add sync pair
UPDATED=$(jq --arg label "${DISPLAY_NAME} Patterns" \
...
echo "  2. Fill in language-specific agent instructions in claude/agents/${LANG}-*.md"
echo "  3. Run: make validate && make sync"
