...
For complex dependency graphs, consider `google/wire` (compile-time DI) or `uber-go/fx` (runtime DI).

### TypeScript: Constructor / Factory
...
// Factory function for assembly
function createAuthService(db: Database, logger: Logger): AuthService {
  const users = new PostgresUserRepository(db);
  return new AuthService(users, logger);
}
```

### Python: Constructor / FastAPI Depends
