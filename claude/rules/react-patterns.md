...
## State Management
- **Server state:** Use TanStack Query (React Query) for all server data. Never manually manage loading/error/data states for API calls.
- **Client state:** Use Zustand for global client state. Use `useState` for local component state. Use Context only for truly global, rarely-changing values (theme, locale, auth user).
...
- **Avoid snapshot tests.** They break on every change and provide no useful signal.
- **User-centric queries:** Use `getByRole`, `getByLabelText`, `getByText`. Avoid `getByTestId` unless no semantic alternative exists.
