...
- **Formatting order:** Always order by visibility -- public first, then private (prefixed with `_`).
...
- **Logging:** Use `structlog` or `logging` with JSON formatter for structured logging. Never log secrets or PII.
- **Async:** Use `asyncio` for concurrent I/O. Prefer `async/await` over threading for I/O-bound work.
- **Concurrency safety:** Use `asyncio.Lock` for shared async state. Use `ThreadPoolExecutor` for CPU-bound work. Goroutine-like lifecycle: clear startup, shutdown, and cancellation.
- **Global state:** No module-level mutable state. Use dependency injection instead.
...
## 🧪 Async Testing
- **Plugin:** Use `pytest-asyncio` for async test support. Install: `pip install pytest-asyncio`.
...
- **Auto mode:** Set `asyncio_mode = "auto"` in `pyproject.toml` to avoid marking every test:
...
- **Async fixtures:** Use `async with` in fixtures for async resource management:
...
- **Timeout:** Use `pytest-timeout` to catch hung async tests: `pytest --timeout=10`.
