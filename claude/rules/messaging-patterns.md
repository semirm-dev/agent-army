<!-- Sync: Must stay in sync with cursor/506-messaging.mdc -->
...
- **Implementation:** Check if message was already processed before executing. Use a processed-messages table or Redis SET with TTL.
...
### Per-Platform DLQ Setup

- **SQS:** Configure `RedrivePolicy` with `maxReceiveCount` and `deadLetterTargetArn`.
- **RabbitMQ:** Use `x-dead-letter-exchange` and `x-dead-letter-routing-key` on queue declaration.
- **Kafka:** Publish failed messages to a `<topic>.dlq` topic. Use a separate consumer group for DLQ processing.
...
- **Envelope pattern:** Wrap every event in a standard envelope:
  ...
  ```
- **Versioning:** Include `version` field. Support at least N-1 versions. Use schema registry for Avro/Protobuf.
...
- **Ordering:** Use partition keys (Kafka) or FIFO queues (SQS FIFO) when message order matters within an entity.

## Per-Language Queue Clients
...
- **Giant messages:** Keep messages small (< 256KB). Store large payloads in S3/blob storage and pass a reference.
