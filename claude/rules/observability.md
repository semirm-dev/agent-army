...
- **SDK initialization:** Initialize OTel SDK early in application startup. Configure exporter (OTLP gRPC to collector, or direct to backend).
- **Auto-instrumentation:** Use language-specific auto-instrumentation for common libraries:
  - **Go:** `go.opentelemetry.io/contrib/instrumentation/` packages (net/http, gRPC, database/sql)
  - **TypeScript:** `@opentelemetry/auto-instrumentations-node` for Express, Fastify, pg, redis
  - **Python:** `opentelemetry-instrumentation` packages for Flask, FastAPI, SQLAlchemy, requests
- **Custom spans:** Name spans as `{service}.{operation}` in verb form (e.g., `auth.validateToken`, `orders.processPayment`).
- **Span attributes:** Add business context (`user.id`, `order.id`, `tenant.id`). Never add PII (email, name, IP) as span attributes.
- **Context propagation:** Use W3C TraceContext (`traceparent`, `tracestate` headers). Propagate context across HTTP, gRPC, and message queue boundaries.
