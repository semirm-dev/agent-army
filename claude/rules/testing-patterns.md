...
- **Go:** `[]struct` with `t.Run()` subtests (see `go-patterns.md`)
- **TypeScript:** Array of case objects with `for...of` (see `ts-patterns.md`)
- **Python:** `@pytest.mark.parametrize` (see `py-patterns.md`)
...
- **Explicit timeouts.** Set test timeouts to catch hangs: `go test -timeout 30s`, vitest: `testTimeout` in config, jest: `--testTimeout 10000`.
