...
| Target | Description |
|--------|-------------|
| `make build` | Build the army CLI binary (`army/army`) |
...
`make v1 bootstrap` and pick Claude Code, generates platform-specific output in `.build/claude/` (adjustable):
...
`make v1 bootstrap` and pick Cursor, generates platform-specific output in `.build/cursor/` (adjustable):
