...
### Skills (`army/spec/skills/`)

Structured workflows, decision trees, and coding standards. Markdown files with YAML frontmatter in `army/spec/skills/`. Skills define **how** to accomplish tasks and **what** good code looks like — designing APIs, setting up caching, hardening security, naming conventions, error handling, testing patterns, etc. Skills can depend on other skills via `uses_skills`.

Examples: `api-design`, `caching-strategy`, `go/coder`, `react/tester`

### Agents (`army/spec/agents/`)
...
| `make v1 manifest` | Scan `army/spec/` frontmatter and regenerate `army/manifest.json`. Resolves `uses_skills` and `delegates_to` transitively |
| `make v1 resolve-deps` | Validate all dependency references across `army/spec/`. Detect and remove redundant `uses_skills` and `delegates_to` entries covered by transitive dependencies |
| `make v1 bootstrap` | Generate model-specific skills and agents (output in `army/.build/`) |
| `make v1 sync` | Install all plugins and skills listed in `army/PLUGINS_AND_SKILLS.md` |
| `make v1 update-plugins-skills` | Regenerate `army/PLUGINS_AND_SKILLS.md` from installed system state |
...
1. **`make v1 update-plugins-skills`** — Scans installed Claude Code plugins and standalone skills, writes `army/PLUGINS_AND_SKILLS.md`, and flags redundant standalone skills already covered by plugins.
2. **`make v1 sync`** — Reads `army/PLUGINS_AND_SKILLS.md` and installs all listed plugins and skills. Also removes standalone skills flagged as redundant.
...
2. **Capture** — Run `make v1 update-plugins-skills` to record it in `army/PLUGINS_AND_SKILLS.md`
3. **Verify** — Run `make v1 analyze` to confirm no duplicates or conflicts
4. **Sync other devices** — On any other machine, run `make v1 sync` to install the same plugins and skills from `army/PLUGINS_AND_SKILLS.md`

## Manifest (`army/manifest.json`) - for now unused index file
...
- **path** — file path relative to `army/`

Agent entries additionally include: **role**, **access**, **uses_plugins**, **delegates_to**.

Regenerate with `make v1 manifest`.

## Bootstrap Output (`army/.build/`)

### Claude Code

`make bootstrap` and pick Claude Code, generates platform-specific output in `army/.build/claude/` (adjustable):
...
- `settings.json` — Claude Code settings from `army/spec/claude/settings.json`

### Cursor

`make bootstrap` and pick Cursor, generates platform-specific output in `army/.build/cursor/` (adjustable):
